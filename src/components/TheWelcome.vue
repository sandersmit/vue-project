...
function checkAllColumnStates(colNumber: number) {
  ...
  // }
  console.log("columnIndexRef.value", dataNumberStore.allColumnStates[columnIndexRef.value])
...
//watchers
watch([computeHighlights, computeCounter, computeHaltCount, computeAllRowStates], () => {
  //console.log('watcher')
})
...
<template>
  ...
  computeAllRowStates( {{ computeAllRowStates.length }} ): {{ computeAllRowStates }} <br>
  compute all column states ( {{ computeAllColumnStates.length }} ): {{ computeAllColumnStates }} <br>
  ...
  <section>
    ...
    <CellItem v-for="(item, index) in computeAllRowStates" :key="index" :item-Prop="item" :index-Row-Prop="index"
      :highlights-Prop="computeHighlights" :halt-Prop="computeHaltCount" :index-Column-Prop="columnIndexRef"
      :counter-Prop="computeCounter" :clicked-Colpos-Prop="columnIndexRef" :clicked-Rowpos-Prop="rowIndexRef"
